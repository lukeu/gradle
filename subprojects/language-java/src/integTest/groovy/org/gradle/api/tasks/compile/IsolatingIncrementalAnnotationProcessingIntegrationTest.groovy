...
class IsolatingIncrementalAnnotationProcessingIntegrationTest extends AbstractIncrementalAnnotationProcessingIntegrationTest {
    ...
    def "processors must provide an originating element for each source element"() {
        given:
        withProcessor(new NonIncrementalProcessorFixture(true, false, false).withDeclaredType(IncrementalAnnotationProcessorType.ISOLATING))
    ...
    def "processors can't read resources"() {
        given:
        withProcessor(new NonIncrementalProcessorFixture(false, true, false).withDeclaredType(IncrementalAnnotationProcessorType.ISOLATING))
        def a = java "@Thing class A {}"
        outputs.snapshot { succeeds"compileJava" }
    ...
    def "processors can't write resources"() {
        given:
        withProcessor(new NonIncrementalProcessorFixture(false, false, true).withDeclaredType(IncrementalAnnotationProcessorType.ISOLATING))
        def a = java "@Thing class A {}"
        outputs.snapshot { succeeds"compileJava" }
