...
class AggregatingIncrementalAnnotationProcessingIntegrationTest extends AbstractIncrementalAnnotationProcessingIntegrationTest {
    ...
    def "processors can't read resources"() {
        given:
        withProcessor(new NonIncrementalProcessorFixture(false, true, false).withDeclaredType(IncrementalAnnotationProcessorType.AGGREGATING))
    ...
    def "processors can't write resources"() {
        given:
        withProcessor(new NonIncrementalProcessorFixture(false, false, true).withDeclaredType(IncrementalAnnotationProcessorType.AGGREGATING))
        def a = java "@Thing class A {}"
        outputs.snapshot { succeeds"compileJava" }
