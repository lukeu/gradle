...
public class EclipsePlugin extends IdePlugin {
    ...
    private void configureEclipseProject(final ProjectInternal project, final EclipseModel model) {
        TaskProvider<GenerateEclipseProject> task = project.getTasks().createLater(ECLIPSE_PROJECT_TASK_NAME, GenerateEclipseProject.class, new Action<GenerateEclipseProject>() {
            @Override
    ...
    private void configureEclipseClasspath(final Project project, final EclipseModel model) {
        ...
        project.getPlugins().withType(JavaBasePlugin.class, new Action<JavaBasePlugin>() {
            @Override
            public void execute(JavaBasePlugin javaBasePlugin) {
                final TaskProvider<GenerateEclipseClasspath> task = project.getTasks().createLater(ECLIPSE_CP_TASK_NAME, GenerateEclipseClasspath.class, new Action<GenerateEclipseClasspath>() {
                    @Override
    ...
    private static void configureJavaClasspath(final Project project, TaskProvider<GenerateEclipseClasspath> task) {
                    ...
                    public void projectsEvaluated(Gradle gradle) {
                        task.configure(new Action<Task>() {
                            ...
                            public void execute(Task task) {
                                ...
                                List<Dependency> dependencies = Lists.newArrayList(Iterables.filter(Iterables.concat(Iterables.transform(((GenerateEclipseClasspath) task).getClasspath().getPlusConfigurations(), new Function<Configuration, Iterable<Dependency>>() {
                                    @Override
                                ...
                                if (!dependencies.isEmpty()) {
                                    ((GenerateEclipseClasspath) task).getClasspath().getMinusConfigurations().add(project.getConfigurations().detachedConfiguration(dependencies.toArray(new Dependency[0])));
                                }
                            }
                        });
                    }
    ...
    private static <T extends Task> void maybeAddTask(Project project, IdePlugin plugin, String taskName, Class<T> taskType, Action<T> action) {
        ...
        if (tasks.findByName(taskName) != null) {  // TODO: This realize the task
            return;
        }

        TaskProvider<T> task = tasks.createLater(taskName, taskType, action);
